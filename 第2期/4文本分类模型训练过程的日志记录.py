...
def evaluate(model, iterator, criterion):
    ...
    with torch.no_grad():
        for text, label in iterator:
            ...
            pt = np.sum((predicted[i] == 1 and label[i] == 1) for i in range(len(predicted))).item()
            nt = np.sum((predicted[i] == 0 and label[i] == 0) for i in range(len(predicted))).item()
            pf = np.sum((predicted[i] == 1 and label[i] == 0) for i in range(len(predicted))).item()
            nf = np.sum((predicted[i] == 0 and label[i] == 1) for i in range(len(predicted))).item()
            PT += pt
            NT += nt
            PF += pf
            NF += nf
        ...
        precision = PT / (PT + PF) if (PT + PF) > 0.0 else 0.0
        recall = PT / (PT + NF) if (PT + NF) > 0.0 else 0.0
...
def run():
    ...
    for epoch in trange(NUM_EPOCHS):
        ...
        import logging
        logging.basicConfig(filename='training.log',    # 保存的文件名
                            format='%(message)s',       # 格式 %{message}s 表示仅包括日志消息
                            level=logging.INFO,         # 等级设为 “消息”
